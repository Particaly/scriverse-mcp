...
import { describeContractOverview, describeResourceWriteContract } from "./backend-schemas.js";
...
/**
 * 为 scriverse_schema 的成功结果附加写入校验说明：action=show 时给出该类型
 * 逐字段合法格式，action=list 时给出整体校验约定，让模型在写入前拿到准确的
 * 字段规范（与 backend-schemas.ts 的 strict 校验保持同一份事实来源）。
 */
function appendSchemaContract(
  result: CallToolResult,
  args: Record<string, unknown>
): CallToolResult {
  const action = typeof args.action === "string" ? args.action : undefined;
  const type = typeof args.type === "string" ? args.type : undefined;
  const appendix = action === "show" && type !== undefined
    ? describeResourceWriteContract(type)
    : action === "list"
      ? describeContractOverview()
      : undefined;
  const first = result.content[0];
  if (!appendix || !first || first.type !== "text") return result;
  return {
    ...result,
    content: [{ type: "text", text: `${first.text}\n\n${appendix}` }, ...result.content.slice(1)]
  };
}
 ...
 *
 * 工具回调流程：Zod 校验后的参数 -> buildArgs 组装 argv（写操作 input 会先
 * 按服务端字段规范做 strict 校验）-> 子进程执行 CLI -> 结果转换为文本内容。
 * --input 类命令通过标准输入传递 JSON 请求体。
 */
export function createScriverseMcpServer(options: ScriverseMcpServerOptions): { server: McpServer } {
  const server = new McpServer(
    { name: "scriverse-mcp", version: PACKAGE_VERSION },
    {
      instructions: [
        ...
        "先用 scriverse_schema 查看对应资源的字段规范与示例；",
        "create/update 的 input 会按服务端字段规范做 strict 校验，未知/冗余字段会被直接拒绝，不会提交。",
        "写操作会生成版本历史，可用 history 查询、restore 回滚。",
  ...
  for (const tool of tools) {
    server.registerTool(tool.name, {
      ...
      const result = cliRunToToolResult(run, maxOutputChars);
      return tool.name === "scriverse_schema" && !result.isError
        ? appendSchemaContract(result, args as Record<string, unknown>)
        : result;
    });
  }
  return { server };
